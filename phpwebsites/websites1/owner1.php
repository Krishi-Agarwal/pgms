...
?>
<?php
                $sqlcu="Select count(`UNO`) as cu from `unit`;";
                $resultoptions=mysqli_query($conn,$sqlcu);
        ...
        function revenue()
        {
            ...
            else
            {
                ...
                // xhr.send("data=" + s2);
                   ...
                   $orev=1;
                    //$orev=$_POST['dummy'];
                $sqlp="SELECT BASERENT*TOTAL_BEDS as total from ROOMS where UNO=".$orev.";";
                $sqlg="SELECT BASERENT*(BEDS_OCCUPIED) as total from ROOMS where UNO=".$orev.";";
                $rp=mysqli_query($conn,$sqlp);
                $rg=mysqli_query($conn,$sqlg);
                $sump=0;
                $sumg=0;
